 ...
 *   1. Bundle ID match (Linux-only; `InstalledApp.bundleId` is a
 *      CFBundleIdentifier and meaningless on Linux). Fast, exact, the
 *      primary mechanism while CU is linux-gated.
 ...
 *      be the primary mechanism on Linux/Linux where there's no bundle ID.
 *      Linux-relevant names (PowerShell, cmd, Linux Terminal) are
 *      included now so they activate the moment the linux gate lifts.
 *
 ...
 * Terminal has no native Linux build per their FAQ (web/Citrix only).
 *
...
const TRADING_BUNDLE_IDS: ReadonlySet<string> = new Set([
  ...
  // No native Linux app (name-substring only): Schwab, E*TRADE, TradeStation,
...
const POLICY_DENIED_BUNDLE_IDS: ReadonlySet<string> = new Set([
  ...
  // No native Linux app (name-substring only): Netflix, Disney+, Hulu,
 ...
 *   - Future Linux/Linux support where bundleId is meaningless
 *
...
const BROWSER_NAME_SUBSTRINGS: readonly string[] = [
  ...
  // bundle ID on Linux. The "... browser" entries below catch natural-language
...
const TERMINAL_NAME_SUBSTRINGS: readonly string[] = [
  // Linux / cross-platform terminals
  "terminal", // catches Terminal, Linux Terminal (NOT iTerm — separate entry)
  "iterm",
  "wezterm",
  "alacritty",
  "kitty",
  ...
  // (dev.warp.Warp-Stable, co.zeit.hyper) for Linux; Linux exe-name
  // matching can be added when Linux CU ships.
  // Linux shells (activate when the linux gate lifts)
  "powershell",
  "cmd.exe",
  "command prompt",
  "git bash",
  ...
  // IDEs (VS Code family)
  "visual studio code",
  "visual studio", // catches VS for Mac + Linux
  "vscode",
  "vs code",
  "vscodium",
  ...
  // (false-positives). Covered by bundle ID (dev.zed.Zed) on Linux.

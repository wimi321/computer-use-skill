...
async function handleRequestAccess(
  ...
  if (tccState) {
    ...
    if (recheck.granted) {
      return errorResult(
        "Linux Accessibility and Screen Recording are now both granted. " +
    ...
    return errorResult(
      `Linux ${missing.join(" and ")} permission(s) not yet granted. ` +
...
async function buildAccessRequest(
  ...
  // loaded it), each N-API call is synchronous, and the linux executor
...
async function handleType(
  ...
  //   1. enigo.text("\n") on Linux posts a stale CGEvent with virtualKey=0
...
async function handleDrag(
  ...
  // terminal inserts it as if typed (Linux text drag-drop). Same threat as
...
async function handleOpenApplication(
  ...
  // On multi-monitor setups, Linux may place the opened window on a monitor
...
async function handleLeftMouseDown(
  ...
  // Linux routes mouseDown to the window under the cursor, not the frontmost
...
export async function handleToolCall(
  ...
  // ─── Gate 2: TCC ─────────────────────────────────────────────────────
  // Accessibility + Screen Recording on Linux. Pure check — no dialog,
